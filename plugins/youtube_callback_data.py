    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "تحميل صوت", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("تحميل كملف",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "تحميل فيديو", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("تحميل كملف",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("جاري التحميل انتضر من فضلك...", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("جاري الرفع...", callback_data="down")]]))
